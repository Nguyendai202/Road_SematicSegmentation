...
correct_label = tf.compat.v1.placeholder(tf.float32, [None, IMAGE_SHAPE[0], IMAGE_SHAPE[1], NUMBER_OF_CLASSES])# chứa nhãn 
learning_rate = tf.compat.v1.placeholder(tf.float32)
keep_prob = tf.compat.v1.placeholder(tf.float32)#xác suất để một đơn vị nơ-ron được giữ lại (không bị tắt) trong lớp mạng
...
def conv_1x1(layer, layer_name):
  """ Trả về kết quả của việc convolution 1x1 trên một layer """
  return tf.keras.layers.Conv2D(filters=NUMBER_OF_CLASSES,
                                kernel_size=(1, 1),
                                strides=(1, 1),
...
def upsample(layer, k, s, layer_name):
  """ Trả về kết quả của transpose convolution với kernel_size k và strides s """
  # Xem thêm: http://deeplearning.net/software/theano/tutorial/conv_arithmetic.html#transposed-convolution-arithmetic
  return tf.keras.layers.Conv2DTranspose(filters=NUMBER_OF_CLASSES,
                                         kernel_size=(k, k),
                                         strides=(s, s),
                                         padding='same',
                                         name=layer_name)(layer)
...
def run():
  ...
  export_dir = "/content/drive/MyDrive/Computer_vision/B11/exported_model/10092/"
  # A function to get batches
  get_batches_fn = helper.gen_batch_function2(TRAINING_DATA_DIRECTORY, IMAGE_SHAPE)
  ...
  with tf.Session() as session:
    ...
    tf.saved_model.simple_save(session, export_dir, 
                                ...
                                outputs={"decoderlayer_output/BiasAdd:0":model_output})
